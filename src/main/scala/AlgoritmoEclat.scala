...
object AlgoritmoEclat extends App {
  ...
  val scalaFileContentsList = Source.fromFile(file).getLines().toList //Contenuto di tutto il file come lista
  ...
  val listaAlimentiDSTransazioniNonRagg = for {
    listaAlim <- combinazioniAlimenti
    transazione <- listaTransazioniDS
    if (getTransationIDs(listaAlim, transazione))
  } yield (listaAlim, listaTransazioniDS.indexOf(transazione))

  //Raggruppamento della listaAlimentiDSTransazioniNonRagg per ottenere tutte le transazioni riguardo una certa combinazione
  val listaAlimTransaz = listaAlimentiDSTransazioniNonRagg.groupBy(_._1).filter(_._2.size >= minSupport)
  ...
  //println(listaAlimTransazSupport.mkString("\n"))
}