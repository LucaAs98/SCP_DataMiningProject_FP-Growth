...
object FPGrowthDemo extends App {
  ...
  def countItemSet(item: List[String]): Map[String, Int] = {
    val itemSingoli = (item map (x => x -> (dataset count (y => y.contains(x))))).toMap
    println("Item singoli non filtrati:\n" + itemSingoli.mkString("\n"))
    println("-----------------------------------")
  ...
  def freqItemsetCondPB(item: String, pathList: List[(List[String], Int)], firstMapSorted: ListMap[String, Int]): List[(List[String], Int)] = {
    ...
    if (item == "c") {
      println("Item singoli per l'item c:\n" + itemCount.mkString("\n"))
      println("-----------------------------------")
    }
    ...
    if (item == "c") {
      println("Item singoli per l'item c ordinati:\n" + itemMapSorted.mkString("\n"))
      println("-----------------------------------")
    }
    //Otteniamo i path ordinati per le occorrenze
    val orderedPath = condPBSingSort(pathList, itemMapSorted.keys.toList)
    if (item == "c") {
      println("Path per l'item c ordinati:\n" + orderedPath.mkString("\n"))
      println("-----------------------------------")
    }
    ...
    if (item == "c") {
      println("Albero condizionale dell'item c:")
      condTreeItem.printTree()
      println("-----------------------------------")
    }
  ...
  def avviaAlgoritmo(): Map[Set[String], Int] = {
    ...
    println("Albero con transazioni ordinate:")
    newTree.printTree()
    println("-----------------------------------")
    println("Header Table:\n" + newTree.getHt.mkString("\n"))
    println("-----------------------------------")
    ...
    println("Conditional Pattern Base:\n" + conditionalPatternBase.mkString("\n"))
    println("-----------------------------------")
