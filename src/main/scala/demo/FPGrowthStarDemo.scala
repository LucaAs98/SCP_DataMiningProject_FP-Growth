...
import classes.{Node, TreeStar}
import utils.Utils._

import scala.annotation.tailrec
import scala.collection.immutable.ListMap

object FPGrowthStarDemo extends App {
  ...
  def calcFreqItemset(listItem: List[String], tree: TreeStar, accFreqItemset: List[(List[String], Int)]): List[(List[String], Int)] = {
    ...
    if (listItem.nonEmpty) {
      ...
      //Elementi che frequentemente  sono accoppiati con l'item esaminato, li prendiamo dalla matrice
  ...
  def exec() = {
    ...
    println("-----------------------------------")
    println("Matrice albero:\n" + newTree.matrix.map(x => x._1 -> x._2.toList).mkString("\n"))
    println("-----------------------------------")
