...
object FPGrowthParDemo{
  ...
  def exec(minSupport: Int, pathInput:String): (Map[Set[String], Int], Long, Float) = {
    ...
    def freqItemsetCondPBPar(item: String, headList: List[(List[String], Int)], firstMapSorted: ListMap[String, Int]): List[(List[String], Int)] = {
      ...
      if (!condTreeItem.moreBranch) {
        ...
        val freqItemsetItem = condFPTreePar(newCondPB, itemMapSorted)
