...
* quei 3. Così via per le tuple di dimensione maggiore. */
  def avviaIntersezione(transazioniElementiSingoli: ParSet[(Set[String], Set[Int])], itemSingoli: Set[String]): ParMap[Set[String], Set[Int]] = {

    @tailrec
    def intersezione(livelloCombinazione: Int, transazioniTrovate: ParMap[Set[String], Set[Int]]): ParMap[Set[String], Set[Int]] = {
  ...
  def avvia(): ParMap[Set[String], Set[Int]] = {
    ...
    * Nel nostro caso l'ID è l'indice in cui la transazione si trova nel dataset. //(item, lista transazioni), (item, lista transazioni)...*/
    val itemTransNotFiltered = itemSingoli.map(item => (item, transazioniFile.filter(_._2.contains(item)).map(_._1).seq)).par
